...
public class ActivityEditMail extends AppCompatActivity {
    public static Context context;
    public static ImageButton logo;
    public static EditText newMail;
    public static Button btn;
    public static String loggedId = "", mailString = "";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = this;
        logo = (ImageButton) findViewById(R.id.edit_mail_logo);
        btn = (Button) findViewById(R.id.edit_mail_btn);
        newMail = (EditText) findViewById(R.id.edit_mail_);
        if(checkInternet()){
            ...
            btn.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    mailString = newMail.getText().toString();
                    if(mailString.isEmpty()){
                        newMail.setError(getString(R.string.mail_required));
                        return;
                    }
                    if(mailString.contains(" ")){
                        newMail.setError(getString(R.string.no_whitespaces));
                        return;
                    }
                    if(!mailString.contains("@") || !mailString.contains(".")){
                        newMail.setError(getString(R.string.wrong_mail));
                        return;
                    }
                    mailRequest("https://vendpay.ge/user/editMail?id=" + loggedId + "&mail=" + mailString);
                }
            });
        }
        else{
    ...
    private void mailRequest(final String url) {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                ActivityEditMail.this.runOnUiThread(new Runnable() {
                    public void run() {
                        progressDialog.dismiss();
                        if (sb.toString().contains("Error: E-mail")) {
                            newMail.setError(getString(R.string.mail_exists));
                            return;
                        }
